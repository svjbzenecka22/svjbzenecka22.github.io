# Otevřené otázky

## Strategické otázky

1. Má být budoucí web pouze informační, nebo má být postupně portálem pro správu agend SVJ?
2. Které části mají být veřejné a které neveřejné?
3. Kdo bude web dlouhodobě spravovat?
4. Jaký je přijatelný měsíční rozpočet, pokud bezplatná varianta nebude stačit?
5. Má být prioritou rychlost spuštění první agendy, nebo dlouhodobá architektura?

## Otázky k uživatelům a oprávněním

1. Má mít přístup vlastník, nájemník, nebo oba?
2. Bude se pracovat s jednotkou, osobou, nebo domácností?
3. Jak se bude ověřovat oprávnění vyplnit požadavek za jednotku?
4. Jak se řeší jednotka s více spoluvlastníky?
5. Kdo bude mít administrátorský přístup za výbor?

## Otázky k přístupovému systému

1. Má být jedna finální odpověď za byt?
2. Má být možné odpověď po odeslání změnit?
3. Jaký je termín uzávěrky?
4. Jaký je povolený rozsah počtu čipů?
5. Mají se sbírat i kontaktní údaje osoby, která odpověď vyplnila?
6. Má uživatel potvrdit souhlas s doplatkem?
7. Bude doplatek stejný pro všechny varianty, nebo se bude lišit podle telefonu a počtu čipů?
8. Jaký export potřebuje dodavatel?
9. Jaký export potřebuje výbor pro kontrolu plateb?

## Prioritní otázky před výběrem technologie

Tyto otázky je vhodné zodpovědět dříve, než se bude vybírat konkrétní nástroj nebo služba.

### Pravidlo odpovědi za jednotku

1. Kdo je oprávněn odeslat odpověď za jednotku: vlastník, spoluvlastník, nájemník, uživatel bytu, nebo kdokoliv s pověřením?
2. Pokud je jednotka ve spoluvlastnictví, stačí odpověď jednoho spoluvlastníka?
3. Pokud byt užívá nájemník, může nájemník rozhodnout o počtu čipů a typu telefonu bez potvrzení vlastníka?
4. Kdo nese odpovědnost za doplatek, pokud odpověď vyplní nájemník nebo jiná pověřená osoba?
5. Co se stane, když výbor obdrží dvě rozdílné odpovědi za stejnou jednotku?

Pracovní odpovědi z 2026-05-08:

- Odpověď se má v první agendě vázat primárně k bytové jednotce, ne ke konkrétní osobě.
- Odpověď může odeslat osoba, která se za danou jednotku přihlásí do systému nebo použije přístup určený pro danou jednotku.
- V první fázi se nepředpokládá spolehlivé ověření totožnosti osoby proti samostatnému systému vlastníků nebo nájemníků.
- U bytů vlastněných fyzickou osobou zůstává odpovědnost vůči SVJ na vlastníkovi, i když praktický požadavek může zadat jiná osoba.
- U bytů ve vlastnictví Stavebního bytového družstva Mír má být praktickým rozhodovatelem spíše uživatel bytu, protože ten reálně zná potřebu čipů a telefonu.
- U bytů vlastněných fyzickou osobou budou oficiálně oslovováni vlastníci, nikoliv nájemníci.
- U spoluvlastnictví stačí odpověď jedné osoby jednající za jednotku, protože systém v první fázi nebude ověřovat totožnost ani vztah osoby k jednotce.
- Doplatek a platební komunikace se mají u bytů vlastněných fyzickou osobou posílat na kontakt evidovaný u dané jednotky, typicky na vlastníka bytu.
- U nájemních bytů ve vlastnictví Stavebního bytového družstva Mír má platba směřovat na nájemníky; SBD Mír nemá vystupovat jako zprostředkovatel této platby.
- Pokud bude za stejnou jednotku zadáno více odpovědí, nemá jít o více osobních odpovědí, ale o změny odpovědi dané jednotky.
- Do uzávěrky má být možné odpověď za jednotku změnit; pro další práci se předpokládá, že platí poslední odpověď za jednotku před uzávěrkou.
- Do budoucna je preferovaný směr jednotkový účet nebo jiný stabilní přístup za každou jednotku, použitelný i pro další dotazníky, hlasování nebo zprávy.
- Pokud by se u bytu vlastněného fyzickou osobou ozval vlastník i uživatel bytu s rozdílným požadavkem, je pro komunikaci vůči SVJ určující vlastník, protože právě vlastník je oficiálně oslovenou osobou.

### Změny a uzávěrka

1. Má do uzávěrky platit poslední odeslaná odpověď za jednotku?
2. Má být změna odpovědi možná samoobslužně, nebo pouze přes výbor?
3. Bude po uzávěrce ještě možné individuálně změnit odpověď?
4. Kdo schvaluje výjimky po uzávěrce?
5. Jakým způsobem budou kontaktovány jednotky, které do uzávěrky neodpoví?

Pracovní odpovědi z 2026-05-08:

- Do uzávěrky má platit poslední odeslaná odpověď za jednotku.
- Změna odpovědi do uzávěrky má být samoobslužná, tedy bez nutnosti kontaktovat výbor.
- Po uzávěrce už nemá být změna řešena běžnou uživatelskou částí systému.
- Po uzávěrce musí odpovídající osoba kontaktovat výbor individuálně, například e-mailem, telefonicky nebo přes WhatsApp.
- Výjimky po uzávěrce řeší výbor; pokud bude změna přijata, výbor ji zapíše nebo upraví administrativně ze své strany.
- Jednotky, které do uzávěrky neodpoví, budou kontaktovány e-mailem.

### Rozsah údajů

1. Stačí evidovat pouze jednotku, počet čipů a typ telefonu?
2. Má se povinně sbírat jméno osoby, která odpověď vyplnila?
3. Má se povinně sbírat e-mail nebo telefon pro potvrzení a řešení nejasností?
4. Má osoba uvést vztah k jednotce, například vlastník, spoluvlastník, nájemník nebo pověřená osoba?
5. Má být povolená volná poznámka, nebo by zvyšovala riziko sběru zbytečných osobních údajů?

### Čipy a telefony

1. Jaký je minimální a maximální počet čipů na jednotku?
2. Existuje základní počet čipů zahrnutý v ceně, nebo se platí každý čip?
3. Jsou všechny typy bytového telefonu dostupné pro všechny jednotky?
4. Bude možné objednat žádný nový telefon, pokud partaj nechce měnit stávající zařízení?
5. Má výbor nebo dodavatel doporučenou výchozí variantu telefonu pro nerozhodnuté partaje?

### Doplatky a potvrzení

1. Budou ceny čipů a telefonů známé před spuštěním sběru?
2. Pokud ceny nebudou finální, jak přesně má znít potvrzení platební povinnosti?
3. Má formulář zobrazit orientační nebo přesnou výši doplatku?
4. Má být doplatek za čipy a telefon hrazen jednou platbou, nebo odděleně?
5. Jaké údaje má výbor potřebovat pro následnou kontrolu plateb?

### Exporty a kontrola

1. V jakém formátu má výbor předat objednávku dodavateli?
2. Potřebuje dodavatel identifikaci po bytech, po stoupačkách, po vchodech nebo jiném technickém členění?
3. Má export pro výbor obsahovat kontakty a historii změn, nebo jen poslední platnou odpověď?
4. Má být možné snadno vyfiltrovat jednotky bez odpovědi?
5. Jak dlouho se mají odpovědi uchovávat po dokončení objednávky a vyúčtování doplatků?

## Otázky pro dodavatele přístupového systému

1. Jaké přesné typy bytových telefonů lze objednat a jaké jsou jejich ceny?
2. Jaké jsou ceny čipů a případné limity počtu čipů na jednotku?
3. Jaké údaje dodavatel potřebuje pro objednávku a montáž?
4. Je potřeba znát umístění, patro, stoupačku nebo jiné technické členění jednotky?
5. Do kdy musí být dodavateli předán finální souhrn?
6. Lze po předání objednávky ještě měnit počty čipů nebo typ telefonu?
7. Jak se řeší neodpovězené jednotky?

## Otázky pro výbor před komunikací s vlastníky

1. Jaký bude oficiální text výzvy vlastníkům a uživatelům bytů?
2. Jak bude výbor vysvětlovat doplatek a jeho úhradu?
3. Kdo z výboru bude odpovídat na dotazy a řešit opravy odpovědí?
4. Jaký bude náhradní způsob odpovědi pro osoby, které nevyužijí online formulář?
5. Jak bude výbor evidovat odpovědi přijaté mimo online formulář?
6. Kdy bude sběr spuštěn a kdy bude uzávěrka?
7. Kdy proběhne kontrola úplnosti a připomenutí neodpovězeným jednotkám?

## Otázky k dokumentům a hlasování

1. Jaké typy dokumentů budou veřejné?
2. Jaké typy dokumentů budou pouze pro vlastníky?
3. Má web jen podporovat přípravu hlasování, nebo i samotné hlasování?
4. Jsou stanovy SVJ kompatibilní s elektronickým rozhodováním?
5. Bude potřeba uchovávat auditní stopu souhlasů a hlasování?